...
class CleoraRecommender:
    ...
    def fit(self, data_path):
        ...
        self.user_to_items = {u: set(items) for u, items in user_groups.items()}
        self.item_id_to_idx = {item: idx for idx, item in enumerate(self.item_ids)}
    ...
    def ask_for_recommendation(self, user_id, k=10):
        ...
        seen_indices = [self.item_id_to_idx[item] 
        ...
        distances, indices = self.knn_model.kneighbors(
            user_embedding, 
            n_neighbors=k + len(seen_indices)  # Get extra to filter seen items
        )

...
class FunkSVD:
    ...
    def recommend(self, user_id, amount):
        ...
        score_formula = lambda item_id: predicted_ratings[
            item_id
        ]  # * self.score_recommendation_amount(self.count_recommendations[self.reverse_app_index[item_id]]) #* self.score_rating(self.get_game_rating(item_id))
        perfect_games = [(item_id, score_formula(item_id))
                         for item_id in non_interacted_games
                         if score_formula(item_id) == 1]

        if len(perfect_games) >= amount:
            return random.choice(perfect_games, amount)
        non_interacted_ratings = [(item_id, score_formula(item_id))
                                  for item_id in non_interacted_games]
    ...
    def train(
        ...
        if start_over:
            ...
            user_matrix = np.random.normal(0, 0.1,
                                           (self.n_users, latent_features))
            games_matrix = np.random.normal(0, 0.1,
                                            (self.n_games, latent_features))
        else:
        ...
        for epoch in range(num_epochs):
            ...
            loop = tqdm(
                zip(
                    ...
                    self.rating_matrix_sparse.col,
                    self.rating_matrix_sparse.data,
                ),
                total=len(self.rating_matrix_sparse.data),
            )
            for user_idx, game_idx, rating in loop:
                prediction = np.dot(user_matrix[user_idx],
                                    games_matrix[game_idx])
                error = rating - prediction

                user_matrix[user_idx] += learning_rate * (
                    error * games_matrix[game_idx] -
                    regularization * user_matrix[user_idx])
                games_matrix[game_idx] += learning_rate * (
                    error * user_matrix[user_idx] -
                    regularization * games_matrix[game_idx])

                total_error += error**2

            rmse = np.sqrt(total_error / len(self.rating_matrix_sparse.data))
            print(
    ...
    def train_one_user(
        ...
        if user_matrix.shape[1] != latent_features:
            raise ValueError(
                "Latent feature dimension mismatch between loaded model and specified value."
            )
        ...
        for epoch in range(num_epochs):
            ...
            for game_idx, rating in zip(user_ratings.col, user_ratings.data):
                ...
                user_vector += learning_rate * (error * games_matrix[game_idx]
            ...
            print(
                f"User {user_id}, Epoch {epoch + 1}/{num_epochs}, Total Error: {total_error:.4f}"
            )

        user_matrix[user_idx] = user_vector
        self.save_model(user_matrix, games_matrix)

    def save_model(self, user_matrix,
                   games_matrix):  # , user_biases, item_biases, global_mean):
        """
        Save user and games matrices for later loading
        """
        np.save(os.path.join(self.base_dir, self.save_dir, "user_matrix.npy"),
                user_matrix)
        np.save(os.path.join(self.base_dir, self.save_dir, "games_matrix.npy"),
                games_matrix)
    ...
    def load_model(self):
        ...
        """
        user_path = os.path.join(self.base_dir, self.save_dir,
                                 "user_matrix.npy")
        games_path = os.path.join(self.base_dir, self.save_dir,
                                  "games_matrix.npy")
    ...
    def load_mappings(self):
        ...
        user_index_file = os.path.join(self.base_dir, mapping_dir,
                                       "user_index.joblib")
        app_index_file = os.path.join(self.base_dir, mapping_dir,
                                      "app_index.joblib")
        reverse_user_index_file = os.path.join(self.base_dir, mapping_dir,
                                               "reverse_user_index.joblib")
        reverse_app_index_file = os.path.join(self.base_dir, mapping_dir,
                                              "reverse_app_index.joblib")

        os.makedirs(os.path.join(self.base_dir, mapping_dir), exist_ok=True)

        if (os.path.exists(user_index_file) and os.path.exists(app_index_file)
                and os.path.exists(reverse_user_index_file)
                and os.path.exists(reverse_app_index_file)):
            ...
            if self.games is None:
                self.games = pd.read_csv(
                    os.path.join(self.data_directory, "games.csv"))
            if self.users is None:
                self.users = pd.read_csv(
                    os.path.join(self.data_directory, "users.csv"))
            ...
            self.user_index = {
                user_id: idx
                for idx, user_id in enumerate(unique_userid)
            }
            self.app_index = {
                app_id: idx
                for idx, app_id in enumerate(unique_appid)
            }
            self.reverse_user_index = {
                idx: user_id
                for idx, user_id in enumerate(unique_userid)
            }
            self.reverse_app_index = {
                idx: app_id
                for idx, app_id in enumerate(unique_appid)
            }
    ...
    def load_recommendations_count(self):
        ...
        if os.path.exists(file_path):
            self.count_recommendations = np.load(file_path,
    ...
    def count_recommendations_func(self):
        recommendation_file_path = os.path.join(self.data_directory,
                                                "recommendations.csv")
        self.recommendations = pd.read_csv(recommendation_file_path)
        self.count_recommendations = dict()
        loop = tqdm(self.recommendations.iterrows(),
    ...
    def get_recommendation_amount(self, app_id):
        ...
        if not hasattr(self, "recommendations"):
            self.recommendations = pd.read_csv(
                os.path.join(self.data_directory, "recommendations.csv"))
        count = self.recommendations[self.recommendations["app_id"] ==
                                     app_id].shape[0]  # cache
        return count
    ...
    def score_game(self, user_id, app_id, score):
        ...
        new_rating_matrix = coo_matrix(
            (new_data, (new_row_indices, new_col_indices)),
            shape=rating_matrix_sparse.shape,
        )

        rating_matrix_sparse = vstack(
            [rating_matrix_sparse, new_rating_matrix])
        save_npz(
            os.path.join(self.data_directory, "rating_matrix_sparse.npz"),
            rating_matrix_sparse,
        )

    def score_rating(self, rating_name
                     ):  # should be cached - app_id -> rating, takes too long
...
if __name__ == "__main__":
    ...
    # funksvd = FunkSVD(
    #     "../lightFMscaNN/data/train_and_test.npz",
    #     "../lightFMscaNN/data",
    #     save_dir="model_64L",
    # )
    # funksvd.train(
